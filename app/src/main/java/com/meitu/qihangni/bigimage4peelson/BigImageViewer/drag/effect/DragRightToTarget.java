 ...
 */
public class DragRightToTarget implements DragContract.DragCallback {
    ...
    public DragRightToTarget(ViewGroup rootView, @Nullable View backgroundView, @Nullable DragActionListener dragActionListener) {
        mRootView = rootView;
    ...
    public void dragEvent(float startX, float startY, float endX, float endY) {
        ...
        float distance = mRootView.getWidth();
        float move = endX - startX;
    ...
    public void complete(boolean isFastScroll) {
        isDraging = false;
        if (!isFastScroll && mScale >= DEFAULT_CANCLE_SCALE) {
            cancel();
        } else {
            close();
        }
